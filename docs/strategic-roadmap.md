...
Purpose: specialized agents for distinct content needs, each with its own search strategy, curation logic, and output format.
  ...
  **Agent 1: Insights Agent**
    ...
    Searches: 7 queries guaranteed to span different topic buckets: 2 tournament-specific (max),
              1 each from stats/research, business/economics, gear/tech, history/trivia, training/surface
    Output: up to 3 distinct insight cards with AI-generated illustrations (fewer if quality bars not met)
    Dedup: within-batch pairwise check + memory semantic check (0.82 threshold)
    Schedule: daily at 9 AM via cron
    Commands: --generate-insights / --review-insights

  **Agent 2: News Agent**
    Job: find current tennis news worth knowing
    Searches: 6-8 queries split across distinct buckets (results, human/off-court, incidents);
              during multi-tournament grass weeks, runs one query per active event to cover all venues
    Output: up to 3 distinct news cards (fewer if quality bars not met)
    Dedup: within-batch pairwise check + memory semantic check (0.82 threshold)
...
Currently implemented:
  ...
  - Recap deduplication: new recaps checked against published recaps by filename (one per day)
  - Semantic deduplication: all other content types checked against memory.json (RAG, cosine ≥ 0.82) before saving as candidate — covers all lifecycle states (pending, published, rejected); within-batch pairwise check also runs on same-run LLM output to prevent 3 angles of the same story
...
```
Card statuses (tracked in data/tg-review-queue.json):

PENDING → Card generated by agent, not yet reviewed

SENT → Card sent to Telegram for review (buttons: Publish / Later / Reject)
  │       ← written to RAG memory as "pending" on Telegram send
  │
  ├── ✅ Publish → IMAGE GENERATION (insights only, gpt-image-1)
  │                    │
  │                    └── → PUBLISHED
  ...
  │                           • RAG memory status → "published"
  │
  ├── 📅 Later → SAVED_LATER
  │               Card stays in candidates folder
  │               Re-surfaces via --send-pending --include-saved
  │
  └── 🗑 Reject → REJECTED
                    File permanently removed from candidates
                    RAG memory status → "rejected"

Terminal review options: y=publish, n=delete, t=save for later
Telegram review options: ✅ Publish, 📅 Later, 🗑 Reject (inline buttons)
Both paths write to the same queue and the same RAG memory store.

News cards skip the image generation step — admin provides a real photo path after approving the text.
```

#### 4.3.2 RAG Memory Store (memory.py)
```
Flat-file semantic memory at data/memory.json.
Every card (all types) is embedded with text-embedding-3-small (1536d) on:
  - Telegram send (status: "pending")
  - Publish (status: "published")
  - Reject (status: "rejected")

Content types tracked: news, stat, history, gear, form, recap, prediction

Dedup check (is_semantic_duplicate):
  - Called in every candidate-creating path: insights, news, recap (filename), prediction
  - Threshold: cosine similarity ≥ 0.82 → card is blocked before saving
  - Returns closest match with similarity score for logging

update_memory_status() used for status transitions (pending→published, pending→rejected)
...
**Card body structure (enforced June 2026):**
Every card body (news and insights) follows a mandatory 2-3 sentence structure:
sentences 1-2 = tight WHAT (no repetition of title/tags), final sentence = grounded WHY.

The WHY line uses one of two grounding tiers:
- **Computable** (always safe): ranking gap, stage reached, calendar proximity to next Slam,
  tournament tier, any number stated in the source.
- **Recalled** (requires citation): historical comparisons, season records, player sentiment.
  Recalled WHYs must cite a verbatim source phrase in the `why_source` JSON field.

`generate_cards()` runs `_validate_why_source()` on every card: if the cited phrase is not
found in the search content, the field is prefixed `⚠ RECALLED (not found in source):` —
