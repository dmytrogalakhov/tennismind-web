...
MATCH ANALYSIS:
    Admin runs publish_now.py
        ...
        → Website auto-renders from markdown

DAILY INSIGHTS:
    Cron runs generate_feed.py --generate at 9 AM
        → Tavily searches for interesting tennis news
        → Sonnet curates 1-3 candidates
        → Saved to ~/tennismind-web/content/feed-candidates/
    Admin runs generate_feed.py --review
        → Approve/reject each candidate
        → Approved: DALL-E generates image (retro French poster style)
        → Image saved to ~/tennismind-web/public/feed/
